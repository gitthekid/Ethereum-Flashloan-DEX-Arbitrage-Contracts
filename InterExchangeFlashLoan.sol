...
contract MyV2FlashLoan is FlashLoanReceiverBase {
    ...
    function myFlashLoanCall(string memory _buyExchange, string memory _sellExchange, address _token0AddressFLASHTOKEN, address[] memory _buyExchangeTokenArray, address[] memory _sellExchangeTokenArray, uint256 _flashAmount) solamenteOwner() public {
        ...
        bytes memory params = abi.encode(
            ...
            _buyExchangeTokenArray,
            _sellExchangeTokenArray,
            _flashAmount
        );
        ...
        returns (bool)
    {
        ...
        (
            ...
            address[] memory buyExchangeTokenArray,
            address[] memory sellExchangeTokenArray,
            uint256 flashAmount
        ) = abi.decode(params, (string, string, address, address[], address[], uint256));
        ...
        // Buy logic here
        if (keccak256(abi.encodePacked(buyExchange)) == keccak256(abi.encodePacked("uniswap"))) {
            swapOnUniSwap(flashAmount, 0, buyExchangeTokenArray);
        } else if (keccak256(abi.encodePacked(buyExchange)) == keccak256(abi.encodePacked("sushiswap"))) {
            swapOnSushiSwap(flashAmount, 0, buyExchangeTokenArray);
        } else if (keccak256(abi.encodePacked(buyExchange)) == keccak256(abi.encodePacked("shibaswap"))) {
            swapOnShibaSwap(flashAmount, 0, buyExchangeTokenArray);
        } else {
            revert("Invalid buy exchange brah");
        }

        // Sell logic here
        if (keccak256(abi.encodePacked(sellExchange)) == keccak256(abi.encodePacked("uniswap"))) {
            swapOnUniSwap(IERC20(sellExchangeTokenArray[0]).balanceOf(address(this)), 0, sellExchangeTokenArray);
        } else if (keccak256(abi.encodePacked(sellExchange)) == keccak256(abi.encodePacked("sushiswap"))) {
            swapOnSushiSwap(IERC20(sellExchangeTokenArray[0]).balanceOf(address(this)), 0, sellExchangeTokenArray);
        } else if (keccak256(abi.encodePacked(sellExchange)) == keccak256(abi.encodePacked("shibaswap"))) {
            swapOnShibaSwap(IERC20(sellExchangeTokenArray[0]).balanceOf(address(this)), 0, sellExchangeTokenArray);
        } else {
            revert("Invalid sell exchange brah");
        }
